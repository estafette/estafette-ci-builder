...
func TestGenerateEntrypointScript(t *testing.T) {
	...
	t.Run("DoesNotRunCommandsWithExportInBackground", func(t *testing.T) {
		...
		// act
		path, extension, err := dockerRunner.generateEntrypointScript("/bin/sh", []string{"export $(python3 requiredenv.py)", "ls -latr"}, false)
	...
	t.Run("EscapesDoubleQuotesInEchoStatements", func(t *testing.T) {
		...
		// act
		path, extension, err := dockerRunner.generateEntrypointScript("/bin/sh", []string{"echo \"<xml />\""}, false)
