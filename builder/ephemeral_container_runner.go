...
// NewEphemeralContainerRunner returns a new ContainerRunner to run containers using Kubernetes resources
func NewEphemeralContainerRunner(envvarHelper EnvvarHelper, obfuscator Obfuscator, kubeClientset *kubernetes.Clientset, config contracts.BuilderConfig, tailLogsChannel chan contracts.TailLogLine) ContainerRunner {

	namespace, _ := ioutil.ReadFile("/var/run/secrets/kubernetes.io/serviceaccount/namespace")

	return &ephemeralContainerRunnerImpl{
...
type ephemeralContainerRunnerImpl struct {
	envvarHelper    EnvvarHelper
	obfuscator      Obfuscator
...
func (dr *ephemeralContainerRunnerImpl) IsImagePulled(ctx context.Context, stageName string, containerImage string) bool {
	return true
}

func (dr *ephemeralContainerRunnerImpl) PullImage(ctx context.Context, stageName string, containerImage string) (err error) {
	return
}

func (dr *ephemeralContainerRunnerImpl) GetImageSize(containerImage string) (totalSize int64, err error) {
	return totalSize, nil
}

func (dr *ephemeralContainerRunnerImpl) getEphemeralContainerName(stage manifest.EstafetteStage) (podName string) {
	return fmt.Sprintf("%v-%v", stage.Name, generateRandomString(5))
}

func (dr *ephemeralContainerRunnerImpl) StartStageContainer(ctx context.Context, depth int, runIndex int, dir string, envvars map[string]string, stage manifest.EstafetteStage, stageIndex int) (podID string, err error) {
	...
	// TODO in the future use https://kubernetes.io/docs/concepts/workloads/pods/ephemeral-containers/ instead when they're out of alpha
...
func (dr *ephemeralContainerRunnerImpl) StartServiceContainer(ctx context.Context, envvars map[string]string, service manifest.EstafetteService) (podID string, err error) {
	return podID, fmt.Errorf("Service containers are currently not supported for builder.type: kubernetes")
}

func (dr *ephemeralContainerRunnerImpl) RunReadinessProbeContainer(ctx context.Context, parentStage manifest.EstafetteStage, service manifest.EstafetteService, readiness manifest.ReadinessProbe) (err error) {
	return fmt.Errorf("Service containers are currently not supported for builder.type: kubernetes")
}

func (dr *ephemeralContainerRunnerImpl) TailContainerLogs(ctx context.Context, podID, parentStageName, stageName string, stageType contracts.LogType, depth, runIndex int, multiStage *bool) (err error) {
	...
	if len(pod.Status.ContainerStatuses) > 0 && pod.Status.ContainerStatuses[0].State.Terminated != nil {
		...
		// find container with podID for ephemeral containers
...
func (dr *ephemeralContainerRunnerImpl) followEphemeralContainerLogs(ctx context.Context, podID string, parentStageName, stageName string, stageType contracts.LogType, depth, runIndex int) (err error) {
...
func (dr *ephemeralContainerRunnerImpl) waitWhilePodLeavesState(ctx context.Context, pod *v1.Pod, phase v1.PodPhase) (err error) {
...
func (dr *ephemeralContainerRunnerImpl) StopSingleStageServiceContainers(ctx context.Context, parentStage manifest.EstafetteStage) {
...
func (dr *ephemeralContainerRunnerImpl) StopMultiStageServiceContainers(ctx context.Context) {
...
func (dr *ephemeralContainerRunnerImpl) StartDockerDaemon() error {
	// return fmt.Errorf("Docker daemon should not be started for builder.type: kubernetes")
	return nil
}

func (dr *ephemeralContainerRunnerImpl) WaitForDockerDaemon() {
}

func (dr *ephemeralContainerRunnerImpl) CreateDockerClient() error {
	// return fmt.Errorf("Docker client should not be created for builder.type: kubernetes")
	return nil
}

func (dr *ephemeralContainerRunnerImpl) IsTrustedImage(stageName string, containerImage string) bool {
...
func (dr *ephemeralContainerRunnerImpl) HasInjectedCredentials(stageName string, containerImage string) bool {
...
func (dr *ephemeralContainerRunnerImpl) stopContainer(podID string) error {
...
func (dr *ephemeralContainerRunnerImpl) stopContainers(podIDs []string) {
...
func (dr *ephemeralContainerRunnerImpl) StopAllContainers() {
...
func (dr *ephemeralContainerRunnerImpl) addRunningPodID(podIDs []string, podID string) []string {
...
func (dr *ephemeralContainerRunnerImpl) removeRunningPodIDs(podIDs []string, podID string) []string {
...
func (dr *ephemeralContainerRunnerImpl) CreateNetworks(ctx context.Context) error {
	// return fmt.Errorf("Networks are not supported for builder.type: kubernetes")
	return nil
}

func (dr *ephemeralContainerRunnerImpl) DeleteNetworks(ctx context.Context) error {
	// return fmt.Errorf("Networks are not supported for builder.type: kubernetes")
	return nil
}

func (dr *ephemeralContainerRunnerImpl) generateEntrypointScript(shell string, commands []string, runCommandsInForeground bool) (hostPath, mountPath, entrypointFile string, err error) {
...
func (dr *ephemeralContainerRunnerImpl) initContainerStartVariables(shell string, commands []string, runCommandsInForeground bool, customProperties map[string]interface{}, trustedImage *contracts.TrustedImageConfig) (cmd []string, args []string, binds []string, err error) {
	cmd = make([]string, 0)
	args = make([]string, 0)
	binds = make([]string, 0)
...
func (dr *ephemeralContainerRunnerImpl) generateExtensionEnvvars(customProperties map[string]interface{}, envvars map[string]string) (extensionEnvVars map[string]string) {
...
func (dr *ephemeralContainerRunnerImpl) generateCredentialsFiles(trustedImage *contracts.TrustedImageConfig) (hostPath, mountPath string, err error) {
