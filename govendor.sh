govendor init
govendor fetch gopkg.in/yaml.v2
govendor fetch github.com/stretchr/testify/assert@v1.1.4
govendor fetch github.com/olekukonko/tablewriter
govendor fetch github.com/Knetic/govaluate@v3.0.0
govendor fetch github.com/rs/zerolog/...@v1.8.0
govendor fetch github.com/estafette/estafette-ci-manifest@0.1.37
govendor fetch github.com/sethgrid/pester
govendor fetch github.com/alecthomas/kingpin@v2.2.5
govendor fetch github.com/estafette/estafette-ci-crypt@0.0.2
govendor fetch github.com/estafette/estafette-ci-contracts@0.0.69
govendor fetch github.com/docker/docker-ce/components/engine/api@v18.06.1-ce
govendor fetch github.com/docker/docker-ce/components/engine/client@v18.06.1-ce